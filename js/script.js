...
form.addEventListener("submit", function(event) {
    ...
    if (minY < -50 || maxY > 50 || minX < -50 || maxX > 50) {
        invalidInput.textContent = "Error: All numbers must be in the range of -50 to 50.";
    ...
    if (minY > maxY) {
        invalidInput.textContent = "Error: Min Y cannot be greater than max Y.";
        invalidInput.hidden = false;
        return;
    }
    ...
    if (minX > maxX) {
        invalidInput.textContent = "Error: Min X cannot be greater than max X.";
        invalidInput.hidden = false;
        return;
    }
